...
const simple_jsonrpc_1 = require("./simple-jsonrpc");
const ELGEvents_1 = __importDefault(require("./ELGEvents"));
class WaveLinkClient {
    constructor(system) {
        // Helper methods
        this.fixNames = (name, maxlen = 27, suffix = " &hellip;") => {
    ...
    init(system) {
        ...
        this.event = ELGEvents_1.default.eventEmitter();
        this.on = this.event.on;
        this.emit = this.event.emit;
        this.awl = new AppWaveLink();
        this.rpc = new simple_jsonrpc_1.simple_jsonrpc();
        this.websocket = null;
        this.output = null;
        this.mixers = [];
    ...
    adjustVolume(mixerTyp, mixerId, inSlider, vol) {
        ...
        var localVol, streamVol, deltaLinked, isLinked, slider = inSlider;
        if (mixerTyp == "input") {
            this.mixers.forEach((mixer) => {
                if (mixer.mixerId == mixerId) {
    ...
    setVolume(mixerTyp, mixerId, slider, targetVol, delay) {
        var timeLeft = delay;
        var volumeSteps = 0, localVol, streamVol, isNotBlocked = true;
        const mixer = this.getMixer(mixerId);
        var isNotBlocked = mixerTyp == "input"
            ? slider == "local"
    ...
    setOutputMixer() {
        var localVol = this.output.localVolOut, localMute = this.output.isLocalMuteOut, streamVol = this.output.streamVolOut, streamMute = this.output.isStreamMuteOut;
        this.rpc
            .call("setOutputMixer", {
...
exports.default = WaveLinkClient;
//# sourceMappingURL=WaveLinkClient.js.map