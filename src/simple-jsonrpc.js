...
var ERRORS = {
  ...
  DEVICE_NOT_FOUND: {
    code: -31097,
    message:
      " The specified device was not found in the list of available devices.",
  },
}
...
export const simple_jsonrpc = function () {
  var self = this,
    waitingframe = {},
    id = 0,
    dispatcher = {}
      ...
      try {
        ...
        if (request.hasOwnProperty("params")) {
          if (dispatcher[request.method].params == "pass") {
            result = dispatcher[request.method].fn.call(
              dispatcher,
              request.params
            )
          } else if (isArray(request.params)) {
            result = dispatcher[request.method].fn.apply(
              dispatcher,
              request.params
            )
            ...
            if (dispatcher[request.method].params instanceof Array) {
              ...
              if (Object.keys(request.params).length > 0) {
                return _Promise.resolve({
                  ...
                  error: setError(ERRORS.INVALID_PARAMS, {
                    message:
                      "Params: " +
                      Object.keys(request.params).toString() +
                      " not used",
                  }),
                })
              } else {
              ...
              return _Promise.resolve({
                ...
                error: setError(
                  ...
                  "Undeclared arguments of the method " + request.method
                ),
              })
            }
          }
  ...
  self.dispatch = function (functionName, paramsNameFn, fn) {
    ...
    if (isString(functionName) && paramsNameFn == "pass" && isFunction(fn)) {
      ...
      throw new Error(
        "Missing required argument: functionName - string, paramsNameFn - string or function"
      )
    }
  }
  ...
  self.batch = function (requests) {
    ...
    forEach(requests, function (req) {
      if (req.hasOwnProperty("call")) {
        ...
        message.push(
          notification(req.notification.method, req.notification.params)
        )
      }
    })
